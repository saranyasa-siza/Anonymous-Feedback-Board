...
export const Board: React.FC<Readonly<BoardProps>> = ({ boardDeployment$ }) => {
  ...
  // state, and we just need to forward it to the `submitFeedback` method of the `DeployedFeedbackBoardAPI` instance
  ...
  const onPostMessage = useCallback(async () => {
    ...
    try {
      if (deployedBoardAPI) {
        setIsWorking(true);
        await deployedBoardAPI.submitFeedback(messagePrompt);
      }
    } catch (error: unknown) {
  ...
  // Callback to handle the taking down of a message. Again, we simply invoke the `removeFeedback` method
  // of the `DeployedFeedbackBoardAPI` instance.
  const onDeleteMessage = useCallback(async () => {
    try {
      if (deployedBoardAPI) {
        setIsWorking(true);
        await deployedBoardAPI.removeFeedback();
      }
    } catch (error: unknown) {
      setErrorMessage(error instanceof Error ? error.message : String(error));
    } finally {
      ...
      {boardDeployment$ && (
        <React.Fragment>
          ...
          <CardHeader
            avatar={
              boardState ? (
                boardState.state === State.VACANT || (boardState.state === State.OCCUPIED && boardState.isAuthor) ? (
